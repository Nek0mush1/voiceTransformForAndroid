...
- `拼音` / `ABC` / `123`：切换中文拼音、英文键盘和符号键盘。
- `删除`：删除光标前一个字符；长按会连续删除。
- `空格`：插入空格；在拼音未上屏时会提交首个候选。
- `空格`长按：按住开始语音输入，松手自动停止并进入纠错确认。
- `回车`：发送 Enter。
- `中符` / `英符`：在符号键盘中切换中文符号和英文符号。

Android 端已经内置 THUOCL IT 专业词库用于本地候选词；Rime 朙月拼音只用于给专业词生成单字拼音索引，不会把完整 Rime 生僻字词库暴露到候选栏。`jieba` 更适合后端分词，不作为输入法 APK 的本地拼音候选词库。
