...
import com.osfans.trime.voice.VoiceCorrectionController
...
open class TrimeInputMethodService : LifecycleInputMethodService() {
    ...
    private lateinit var voiceCorrectionController: VoiceCorrectionController
    ...
    private suspend fun updateRimeOption(api: RimeApi) {
        try {
            api.setRuntimeOption("soft_cursor", prefs.keyboard.useSoftCursor.getValue())
        } catch (e: Exception) {
            Timber.e(e)
        }
    }
    ...
    override fun onCreate() {
        ...
        voiceCorrectionController = VoiceCorrectionController(this)
        registerReceiver()
        super.onCreate()
        Timber.d("onCreate")
    ...
    override fun onDestroy() {
        ...
        if (::voiceCorrectionController.isInitialized) {
            voiceCorrectionController.cancel()
        }
        inputView = null
        recreateInputViewPrefs.forEach {
    ...
    override fun onComputeInsets(outInsets: Insets) {
        if (inputDeviceManager.isVirtualKeyboard) {
            inputView?.getTouchableTopInWindow(inputViewLocation)
            outInsets.apply {
    ...
    fun toggleVoiceCorrection() {
        rememberVoiceInputConnection()
        voiceCorrectionController.toggleFromVoiceKey()
    }

    fun stopVoiceCorrectionFromKeyRelease() {
        voiceCorrectionController.stopFromVoiceKeyRelease()
    }

    private fun rememberVoiceInputConnection() {
        currentInputConnection?.let {
            voiceInputConnection = it
        }
    }
    ...
    fun showVoiceCorrectionResult(
        ...
        forceShowSelf()
        inputView?.showVoiceCorrectionResult(rawText, correctedText, method)
    }

    fun commitVoiceCorrectionText(text: String): Boolean {
        val ic = voiceInputConnection ?: currentInputConnection ?: return false
        ic.finishComposingText()
        val committed = ic.commitText(text, 1) ||
            (currentInputConnection?.takeIf { it !== ic }?.let {
