...
class InputView(
    ...
    private val voiceResultTitle =
        TextView(context).apply {
            setText(R.string.voice_transform_result_prompt)
            setTextColor(Color.rgb(73, 80, 87))
            setTextSize(android.util.TypedValue.COMPLEX_UNIT_SP, 16f)
            setTypeface(typeface, Typeface.BOLD)
        }

    private val voiceRawText = createVoiceResultTextView()

    private val voiceCorrectedText =
        createVoiceResultTextView().apply {
            setTextColor(Color.rgb(32, 33, 36))
            setTextSize(android.util.TypedValue.COMPLEX_UNIT_SP, 18f)
            setTypeface(typeface, Typeface.BOLD)
        }

    private val voiceMethodText = createVoiceResultTextView()

    private val voiceInsertCorrectedButton = createVoiceResultButton(primary = true)

    private val voiceInsertRawButton = createVoiceResultButton(primary = false)

    private val voiceCancelButton = createVoiceResultButton(primary = false)

    private val voiceResultPanel =
        LinearLayout(context).apply {
            ...
            background =
                GradientDrawable().apply {
                    ...
                    setStroke(dp(1), Color.rgb(218, 221, 225))
                }
            ...
            addView(
                LinearLayout(context).apply {
                    ...
                    addView(voiceInsertCorrectedButton, LinearLayout.LayoutParams(0, dp(48), 1f))
                    addView(
                        voiceInsertRawButton,
    ...
    private fun createVoiceResultTextView() =
        TextView(context).apply {
            setTextColor(Color.rgb(82, 89, 96))
            setTextSize(android.util.TypedValue.COMPLEX_UNIT_SP, 15f)
            setLineSpacing(dp(2).toFloat(), 1f)
        }

    private fun createVoiceResultButton(primary: Boolean) =
        TextView(context).apply {
            gravity = Gravity.CENTER
            isClickable = true
            isFocusable = false
    ...
    private fun setVoiceResultAction(
        view: TextView,
        action: () -> Unit,
    ) {
    ...
    fun showVoiceCorrectionResult(
        rawText: String,
        correctedText: String,
        method: String,
    ) {
        val safeRaw = rawText.ifBlank { service.getString(R.string.voice_transform_empty_text) }
        val safeCorrected = correctedText.ifBlank { safeRaw }
        voiceRawText.text = service.getString(R.string.voice_transform_raw_display, safeRaw)
        voiceCorrectedText.text = service.getString(R.string.voice_transform_corrected_display, safeCorrected)
        voiceMethodText.text = service.getString(R.string.voice_transform_method_display, method)
        voiceInsertCorrectedButton.setText(R.string.voice_transform_insert_corrected)
        setVoiceResultAction(voiceInsertCorrectedButton) {
            val text = correctedText.ifBlank { rawText }
            if (text.isNotBlank() && service.commitVoiceCorrectionText(text)) {
                hideVoiceCorrectionResult()
            }
        }
        voiceInsertRawButton.setText(R.string.voice_transform_insert_raw)
        setVoiceResultAction(voiceInsertRawButton) {
            if (rawText.isNotBlank() && service.commitVoiceCorrectionText(rawText)) {
                hideVoiceCorrectionResult()
            }
        }
        voiceCancelButton.setText(android.R.string.cancel)
        setVoiceResultAction(voiceCancelButton) {
            hideVoiceCorrectionResult()
        }
        voiceResultPanel.visibility = View.VISIBLE
        voiceResultPanel.bringToFront()
        requestApplyInsets()
    }

    fun hideVoiceCorrectionResult() {
        voiceResultPanel.visibility = View.GONE
        requestApplyInsets()
    }

    fun getTouchableTopInWindow(location: IntArray) {
        val topView = if (voiceResultPanel.visibility == View.VISIBLE) voiceResultPanel else keyboardView
